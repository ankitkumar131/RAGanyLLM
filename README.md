...
- 🪄 **First-run wizard** — the first time you open it, a 4-step setup guides you: check the AI engine (install links if missing), pick what your AI is for (Document Q&A / Website docs / Notes — each pre-tunes retrieval), add files or sample docs, then try a question with one click. Re-run anytime from ⚙️ Settings → Setup Assistant.
- 🔎 **Hybrid Search + advanced retrieval**: ⚙️ Settings → switch from pure semantic search to **Hybrid** (BM25 keywords + semantic vectors, fused) to catch exact names/versions/identifiers. Optional add-ons (default-off in Settings → Retrieval): **rerank** results with the model over a wider candidate pool, **query expansion** (paraphrase → several searches → merged hits), **HyDE** (retrieve with a hypothetical-answer embedding), and **metadata filters** to search only chosen documents/sources. Every response reports which enhancements applied.
- 🗎 **Plain exports + reports** — besides `.raganyllm` packs: ⬇ **Markdown**, **JSONL**, **CSV**, or a printable **HTML report** of the whole knowledge base, all from the KB card.
- 🧠 **Model-aware prompts** — the RAG system prompt adapts to your chosen model family (Llama/Qwen/Gemma/Mistral/DeepSeek/…) and your Simple-mode **"How much detail?"** pick (Concise/Balanced/Detailed); every answer reports which family template it used.
- 📦 **Portable Knowledge Packs (`.raganyllm`)**: One click **⬆ Export KB** saves your whole knowledge base — content *and* embeddings — into a single **ZIP container** (`manifest.json` with sha-256 checksums + `chunks.jsonl`/`embeddings.jsonl`/`settings.json`/AI cards; tampered or altered packs are detected and refused). On any other device, **⬇ Import KB**, choose **Merge** (duplicates skipped) or **Replace**, and you're done. Compact (text-only) exports are also supported and re-learn embeddings on import via Ollama. **🔒 Protect a pack with a password** before sharing — imports then ask for the password (AES-256-GCM). Legacy v1 JSON packs still import.
- 🔗 **LAN share (device-to-device)**: **Share KB** / **Share AI Pack** shows a single-use link (with QR code) that auto-expires — share it with another raganyllm on your Wi-Fi and click its KB-panel **Import from link** (paste the address) to pull the pack straight in, or open the link in a browser for a one-shot download. No cloud, no accounts; the sender's knowledge base is never touched. Requires starting with `HOST=0.0.0.0` for other machines to reach this one.
- 🤖 **AI Packs**: build a custom AI in **Model Forge** (e.g. `menu-bot`), then click **🤖 Export AI Pack** — one file carries your knowledge *and* your custom AIs (name, base model, rules). Import on any device and **⟳ Rebuild** each AI from the local knowledge base; everything dedupes on re-import. Header ⬆ Export KB / ⬇ Import Pack buttons and drag & drop anywhere open a **preview card** (contents + your AIs) before importing; AI-pack imports that need a base model this device lacks offer **⬇ Download now** with live progress.
...
- 💾 **Auto-backups & restore** — Clear KB and Replace-imports snapshot your knowledge base first, and a scheduler snapshots it while the app runs (every 30 min by default, `RAGANYLLM_BACKUP_MINUTES` to tune; unchanged KBs are skipped). Rotating keep-last-10 in `~/.raganyllm/backups`; a 💾 **Back up now** button and timestamped **restore** list sit beside Export/Import KB.
