...
- [x] **Grounded citations** — sources are clickable/expandable, and answer sentences are mapped to the source chunks that support them. The RAG system prompt (§2.3) tells the model to end each fact-using sentence with its source number in brackets ("…heat evenly. [2]"); `lib/grounding.js` then *verifies every claim* against the cited chunk — sentence-token overlap vs. the chunk, markers out of range or with no lexical support are stripped so the UI never shows a fabricated citation, and bracketed numbers that are ordinary prose ("options [1] and [2]") are left untouched. Verified markers stream back as a `citations` SSE event (and as `answer`/`citations`/`citation_markers` in non-stream JSON, where the answer is served marker-free) and render as clickable chips that reveal + flash the matching source card. *(done — per-sentence tags in a separate side view remain polish; citation markers depend on the model following the instruction, and unverified markers are silently dropped)*
