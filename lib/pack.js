...
function normalizePackChunks(rawChunks) {
  ...
  for (let i = 0; i < rawChunks.length; i++) {
    ...
    // §2.1 heading metadata travels inside packs and survives re-imports.
    if (typeof c.heading === 'string' && c.heading) doc.heading = c.heading.slice(0, 300);
    if (typeof c.heading_path === 'string' && c.heading_path) doc.heading_path = c.heading_path.slice(0, 600);
    docs.push(doc);
  }
  return { ok: true, docs };
}
