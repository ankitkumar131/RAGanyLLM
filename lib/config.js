...
const defaultConfig = {
  ...
  search_mode: 'vector',
  // §2.2 advanced retrieval enhancements (all default-off; per-request
  // booleans in /api/query override these)
...
function saveConfig(newConfig) {
  try {
    ...
    // search_mode is an enum — invalid values are ignored at merge time so a
    // partial/bogus update never wipes a previously saved mode.
    if (clean.search_mode !== undefined && clean.search_mode !== 'vector' && clean.search_mode !== 'hybrid') {
      delete clean.search_mode;
    }
    // Advanced-retrieval toggles are booleans — coerce anything truthy/falsy
    // so partial updates or stringy HTTP bodies never store junk.
    for (const k of ['rerank_enabled', 'query_expansion_enabled', 'hyde_enabled']) {
      if (clean[k] !== undefined) clean[k] = !!clean[k];
    }
    ...
    // Guarantee the stored config always has a valid enum (heals legacy files).
    if (updated.search_mode !== 'vector' && updated.search_mode !== 'hybrid') {
      updated.search_mode = defaultConfig.search_mode;
    }
